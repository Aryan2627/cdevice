...
import {
  Send, Camera, Image as ImageIcon, Sparkles, X, Bot, Terminal, LogOut, Volume2, VolumeX
...
const LoginScreen = ({ onLogin }: { onLogin: (token: string) => void }) => {
  ...
  return (
    <LinearGradient colors={['#020617', '#0f172a', '#1e293b']} style={{ flex: 1, paddingTop: insets.top, paddingBottom: insets.bottom, paddingHorizontal: 24, justifyContent: 'center' }}>
...
function MainApp() {
  ...
  if (isCameraActive) {
    if (!permission?.granted) {
      return (
        <View style={{ flex: 1, backgroundColor: '#0f172a', alignItems: 'center', justifyContent: 'center', paddingTop: insets.top, paddingBottom: insets.bottom }}>
  ...
  return (
    <LinearGradient colors={['#020617', '#0f172a', '#020617']} style={styles.container}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        ...
        <BlurView intensity={50} tint="dark" style={[styles.header, { paddingTop: insets.top + 16, paddingBottom: 16 }]}>
        ...
        {showCommands && (
          <BlurView intensity={90} tint="dark" style={[styles.commandMenu, { bottom: Math.max(insets.bottom, 16) + 70 }]}>
        ...
        <BlurView intensity={30} tint="dark" style={[styles.inputContainer, { paddingBottom: Math.max(insets.bottom, 16), paddingTop: 16 }]}>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 20, borderBottomWidth: 1, borderBottomColor: "rgba(255,255,255,0.05)" },
  ...
  commandMenu: { position: 'absolute', left: 16, right: 16, borderRadius: 24, zIndex: 10, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
